...
export default function ContactHeroSection({ page }: { page?: WebsitePageRecord | null }) {
  const heroEyebrow = page?.heroEyebrow || "Get In Touch";
  const heroTitle = page?.heroTitle || "Contact Us";
  const heroSubtitle = page?.heroSubtitle || "";
  const heroBody = page?.heroBody || "";
  const heroImage = page?.heroImage || "/hero.png";
  ...
  return (
    <section className="relative left-1/2 right-1/2 -mx-[50vw] -mt-8 w-screen overflow-hidden sm:-mt-10">
      <div className="relative min-h-[92vh] w-full">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.3)_0%,rgba(0,0,0,0.2)_28%,rgba(0,0,0,0.68)_100%)]" />
